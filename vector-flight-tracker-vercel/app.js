...
function hasLivePosition(flight) {
  return Number.isFinite(flight.live?.latitude) && Number.isFinite(flight.live?.longitude);
}
  ...
  return `
    <article class="flight-card ${hasLivePosition(flight) ? "has-live-position" : ""}" role="button" tabindex="0" data-flight-index="${index}" aria-label="Open ${escapeHTML(flight.flight_iata)} flight position">
      ...
      <div class="gate">
        ...
        <span class="card-action">${hasLivePosition(flight) ? "Track live" : "View details"} <b aria-hidden="true">↗</b></span>
      </div>
    </article>`;
}
...
function altitudeLabel(value, isGround) {
  if (isGround) return "On ground";
...
function openFlightDetails(index) {
  ...
  const positioned = hasLivePosition(flight);
  ...
  $("#detail-altitude").textContent = altitudeLabel(live.altitude, live.is_ground);
  $("#detail-speed").textContent = Number.isFinite(live.speed_horizontal) ? `${Math.round(live.speed_horizontal).toLocaleString()} km/h` : "Not reported";
  $("#detail-heading").textContent = headingLabel(live.direction);
  $("#detail-aircraft").textContent = flight.aircraft?.registration || flight.aircraft?.icao || "Not reported";
  $("#detail-status").textContent = statusLabel(flight.status);
  $("#position-updated").textContent = positioned ? `Updated ${dateTimeLabel(live.updated)}` : "No current coordinate report";
  $("#position-message").textContent = positioned
    ? `The marker shows Aviationstack’s latest reported position for ${flight.flight_iata}.`
    : "Aviationstack has not reported a live position for this flight. Scheduled, landed, and some active flights may not include coordinates.";
  $("#globe-live-badge").textContent = positioned ? "LIVE POSITION" : "POSITION UNAVAILABLE";
  $("#globe-live-badge").classList.toggle("unavailable", !positioned);
  $("#center-aircraft").disabled = !positioned;
  globe.setPosition(positioned ? live : null);
...
flightDialog.addEventListener("close", () => globe.stop());
