...
class FlightGlobe {
  ...
  draw(timestamp = 0) {
    ...
    this.worldLines.forEach((line) => this.strokeGeoLine(line, radius, centerX, centerY, "rgba(255, 248, 204, .9)", 1.45));
...
async function loadWorldOutlines() {
  try {
    const response = await fetch("./assets/world-land.geojson");
    if (!response.ok) return;
    const world = await response.json();
    const lines = (world.features || [])
      .flatMap((feature) => geometryLines(feature.geometry))
      .filter((line) => Array.isArray(line) && line.length > 2);
    globe.setWorldLines(lines);
  } catch {
    // Keep the small built-in outline set when the optional map asset is unavailable.
  }
}
...
const birdSky = $("#bird-sky");
const birdMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
let birdTimer = null;
...
function launchBirdFlock() {
  if (birdMotion.matches || document.hidden) return;
  const fromRight = Math.random() > 0.5;
  const count = Math.floor(randomBetween(1, 4));
  const baseTop = randomBetween(9, 68);

  for (let index = 0; index < count; index += 1) {
    const bird = document.createElement("div");
    bird.className = `flying-bird${fromRight ? " from-right" : ""}`;
    bird.style.setProperty("--bird-top", `${Math.min(78, baseTop + index * randomBetween(3.5, 7))}vh`);
    bird.style.setProperty("--bird-size", `${Math.round(randomBetween(58, 84))}px`);
    bird.style.setProperty("--bird-duration", `${randomBetween(9.5, 14).toFixed(2)}s`);
    bird.style.setProperty("--bird-delay", `${(index * randomBetween(0.3, 0.58)).toFixed(2)}s`);
    bird.style.setProperty("--bird-arc", `${Math.round(randomBetween(-48, 36))}px`);
    bird.innerHTML = '<img src="assets/flying-bird.png" alt="" draggable="false">';
    bird.addEventListener("animationend", (event) => {
      if (event.target === bird) bird.remove();
    });
    birdSky.appendChild(bird);
  }
}

function scheduleBirds(delay = randomBetween(6500, 14500)) {
  window.clearTimeout(birdTimer);
  if (birdMotion.matches) return;
...
birdMotion.addEventListener("change", () => {
  birdSky.replaceChildren();
  scheduleBirds(birdMotion.matches ? undefined : 900);
});
